...
def actor_loss():
    def loss(advantage, predicted_output):
        ...
        loss = -K.sum(advantage * log_probs)
        #actor_loss = loss + 0.01 * K.sum(predicted_output * K.log(predicted_output + 1e-10), axis=1)
        return loss
...
def critic_loss():
    def loss(advantage, predicted_output):
        ...
        ################################
        #raise NotImplementedError
        #critic_loss = K.mean(K.square(advantage - predicted_output))
        critic_loss = 0.5 * K.mean((advantage - predicted_output) ** 2)
        #critic_loss = -advantage * predicted_output
        return critic_loss
...
class A2C(AbstractSolver):
    ...
    def train_episode(self):
        ...
        for _ in range(self.options.steps):
            ...
            if done:
                delta = reward - self.actor_critic.predict([[state]])[1][0]
                deltas.append(delta)
                break
            else:
                delta = reward + self.options.gamma * self.actor_critic.predict([[next_state]])[1][0] - self.actor_critic.predict([[state]])[1][0]
                deltas.append(delta)
            state = next_state
        ################################
        #   YOUR IMPLEMENTATION HERE   #
        ...
        #discounted_r, cumulative_r = np.zeros(len(self.trajectory)), 0
        #for t in reversed(range(0, len(self.trajectory))):
        #    cumulative_r = self.trajectory[t][3] + cumulative_r * self.options.gamma
        #    discounted_r[t] = cumulative_r
        #states = np.array([tr[0] for tr in self.trajectory])
        #next_states = np.array([tr[2] for tr in self.trajectory])
        #actions = [np.eye(self.env.action_space.n)[tr[1]] for tr in self.trajectory]
        #rewards = [tr[3] for tr in self.trajectory]
        #target_reward = [cumulative_r for i in range(len(self.trajectory))]
        #action_probs, states_values = self.actor_critic.predict([states, np.zeros(states.shape[0], 1)])
        #_, next_states_values = self.actor_critic.predict([next_states, np.zeros(next_states.shape[0], 1)])
        #states_values = states_values[0]
        #next_states_values = next_states_values[0]
        #deltas = discounted_r + self.options.gamma * next_states_values - states_values
